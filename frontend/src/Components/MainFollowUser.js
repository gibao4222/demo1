...
function MainFollowUser() {
  ...
  const [followersLoading, setFollowersLoading] = useState(true); // Thêm state
  ...
  // Hàm xử lý chuyển hướng khi nhấn vào số người theo dõi
      ...
      <div className="flex-1 p-8">
        <div className="relative">
          ...
          <div className="absolute bottom-4 left-4">
            ...
            <h1 className="text-6xl font-bold">{user.username || user.email}</h1>
            {followersLoading ? (
              <p className="text-gray-400">Đang tải số người theo dõi...</p>
            ) : (
              <p
