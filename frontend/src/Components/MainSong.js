...
function MainSong() {
  const [songs, setSongs] = useState(() => {
    const savedSongs = localStorage.getItem("recentSongSearches");
  ...
  const fetchSongs = debounce(async (term) => {
    setLoading(true);
    try {
      const url = term.trim()
      ...
      if (Array.isArray(response.data)) {
        ...
        localStorage.setItem("recentSongSearches", JSON.stringify(response.data));
      } else {
  ...
  useEffect(() => {
    ...
    return () => fetchSongs.cancel(); 
  }, [searchTerm]);

  const handleRemoveSong = (songId) => {
    const updatedSongs = songs.filter((song) => song.id !== songId);
    setSongs(updatedSongs);
    localStorage.setItem("recentSongSearches", JSON.stringify(updatedSongs));
  };
  ...
  return (
    <div className="w-3/5 flex-1 p-4 overflow-y-auto">
      ...
      <div className="mb-8">
        <h2 className="text-2xl mb-4">Recent song searches</h2>
        {loading ? (
          <div>Đang tải...</div>
        ) : songs.length === 0 ? (
