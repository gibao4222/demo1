...
const PlaylistDetail = ({ playlist }) => {
    ...
    const handleAddSong = async (songId) => {
        if (!newPlaylist.id || !token) {
            toast.error('Không thể thêm bài hát: thiếu thông tin playlist hoặc token.');
            return;
        }
        ...
        try {
            ...
            setSearchTerm('');
            setSearchResults([]);
            // Phát sự kiện songsUpdated để thông báo làm mới danh sách bài hát
            ...
            if (error.response?.data?.error) {
                ...
                toast.error('Có lỗi xảy ra khi thêm bài hát. Vui lòng thử lại.');
            }
        }
    };
    ...
    const handleDelete = async () => {
        try {
            ...
            toast.error('Có lỗi xảy ra khi xóa playlist. Vui lòng thử lại.');
        }
    };
